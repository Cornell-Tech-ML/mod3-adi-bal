...
def _tensor_matrix_multiply(
    ...
    * Only write to global memory once per kernel.
    ...
    for tile_index in range(num_tiles):
        a_valid = row_index < a_shape[-2] and tile_index * BLOCK_DIM + thread_index_y < a_shape[-1]
        b_valid = tile_index * BLOCK_DIM + thread_index_x < b_shape[-2] and col_index < b_shape[-1]
        
        a_shared[thread_index_x, thread_index_y] = a_storage[
            batch * a_batch_stride + row_index * a_strides[-2] + (tile_index * BLOCK_DIM + thread_index_y) * a_strides[-1]
        ] if a_valid else 0.0
        
        b_shared[thread_index_x, thread_index_y] = b_storage[
            batch * b_batch_stride + col_index * b_strides[-1] + (tile_index * BLOCK_DIM + thread_index_x) * b_strides[-2]
        ] if b_valid else 0.0
        
        cuda.syncthreads()
        
        for k in range(BLOCK_DIM):
            accumulated_value += a_shared[thread_index_x, k] * b_shared[k, thread_index_y]
        
        cuda.syncthreads()
    
    if row_index < out_shape[-2] and col_index < out_shape[-1]:
        out_position = batch * out_strides[0] + row_index * out_strides[1] + col_index * out_strides[2]
